...
export default class UserService {
  async login(userName: string, password: string) {
    if (userName === "admin" && password === "123456") {
      ...
      return Result.error401("账号或密码错误");
    }
  }
  async getMenuByRole(roleId: string) {
    if (roleId === 'super') {
      return Result.success(superMenus);
    } else if (roleId === 'test') {
      return Result.success(testMenus);
    } else {
      return Result.error404("菜单不存在");
    }
  }
}
